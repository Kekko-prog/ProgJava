...
import it.unina.prog.gui.panels.customer.*;
import it.unina.prog.gui.panels.employee.*;

import javax.swing.*;
import java.awt.*;

/**
 * Finestra principale dell'applicazione Swing.
 * Responsabile di:
 * - Gestire il flusso di login (cliente vs dipendente)
 * - Switchare tra diverse viste (CardLayout)
 * - Mantiene l'header con info sessione e bottone logout
 * - Instanzia e gestisce tutti i panel secondari
 * 
 * Usa CardLayout per far visualizzare solo una "card" (vista) alla volta,
 * permettendo transizioni fluide senza aprire o chiudere finestre.
 */
public class ConcessionarioFrame extends JFrame {

	// Costanti per identificare le "card" nel CardLayout
	private static final String CARD_LOGIN = "LOGIN";
	private static final String CARD_DIPENDENTE = "DIPENDENTE";
	private static final String CARD_CLIENTE = "CLIENTE";

	// Layout e pannello principale che ospita le 3 viste alternative
	private final CardLayout cardLayout = new CardLayout();
	private final JPanel cardPanel = new JPanel(cardLayout);
	// Componenti header: label per info sessione e bottone logout
	private final JLabel sessionLabel = new JLabel(" ");
	private final JButton logoutButton = new JButton("Logout");
	// Controller unico che orchestra tutte le operazioni
	private final ConcessionarioController controller = new ConcessionarioController();

	// Cache delle viste dinamiche (create al login, riutilizzate finché l'utente non fa logout)
	private JPanel dipendenteView;
	private JPanel clienteView;

	/**
	 * Costruttore: setup dell'intera interfaccia grafica.
	 * Configura dimensioni, layout, aggiunge header e pannello CardLayout iniziale.
	 */
	...
	/**
	 * Crea l'header della finestra con titolo, info sessione e logout.
	 */
	...
	/**
	 * Crea il pannello di login con scelta del ruolo (cliente/dipendente)
	 * e campi credenziali appropri.
	 */
	...
	/**
	 * Crea l'area di lavoro per un dipendente autenticato.
	 * Usa JTabbedPane per organizz are le operazioni in tab:
	 * Clienti, Veicoli, Vendita, Test Drive, Ordini
	 */
	private JPanel buildDipendenteArea(int dipendenteId) {
		// Area completa per il personale interno.
		JTabbedPane tabs = new JTabbedPane();
		tabs.addTab("Clienti", new ClientiPanel(controller));
		tabs.addTab("Veicoli", new VeicoliPanel(controller));
		tabs.addTab("Vendita", new VenditaPanel(controller, dipendenteId));
		tabs.addTab("Test Drive", new TestDrivePanel(controller));
		tabs.addTab("Ordini", new OrdiniPanel(controller));
	...
	/**
	 * Crea l'area di lavoro per un cliente autenticato.
	 * Accesso limitato a: Profilo, Catalogo, Prenota Test Drive, Le Mie Operazioni
	 */
	private JPanel buildClienteArea(Cliente cliente) {
		// Area limitata alle operazioni disponibili per il cliente.
		JTabbedPane tabs = new JTabbedPane();
		tabs.addTab("Profilo", new ProfiloClientePanel(controller, cliente.getId()));
		tabs.addTab("Catalogo", new CatalogoClientePanel(controller));
		tabs.addTab("Prenota Test Drive", new TestDriveClientePanel(controller, cliente.getId()));
		tabs.addTab("Le Mie Operazioni", new OperazioniClientePanel(controller, cliente.getId()));
	...
	/**
	 * Ritorna al pannello di login e resetta lo stato della sessione.
	 */
	...
	/**
	 * Passa alla vista dipendente: costruisce area tabbed e aggiorna session label.
	 * Rimuove la vista precedente da CardLayout se esiste per evitar duplicati.
	 */
	...
	/**
	 * Passa alla vista cliente: costruisce area tabbed con operazioni client-side
	 * e aggiorna session label.
	 */
	...
	/**
	 * Gestisce il flusso di login di un cliente.
	 * Valida email e password, poi chiama switcher alla relativa vista.
	 */
	private void handleClienteLogin(String email, String password) throws Exception {
		...
		Cliente cliente = controller.autenticaCliente(email, password);
		if (cliente == null) {
	...
	/**
	 * Gestisce il flusso di login di un dipendente.
	 * Valida ID (convertito da string a int) e password, poi switcher alla relativa vista.
	 */
	private void handleDipendenteLogin(String idText, String password) throws Exception {
		...
		Dipendente dip = controller.autenticaDipendente(id, password);
		if (dip == null) {
